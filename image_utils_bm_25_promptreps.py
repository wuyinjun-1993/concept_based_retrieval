...
def load_flickr_dataset(data_path, query_path, subset_img_id=None, redecompose=False):
    ...
    data_path = '/content/unzipped_images_fixed/flickr/'
...
def load_flickr_dataset_full0(data_path, query_path, subset_img_id=None, redecompose=False, total_count = 1000, algebra_method="five"):
    ...
    simple_strings_cleaned = []

    if algebra_method == 'two':
        text_file_path = os.path.join(data_path, "flickr_500_simple_decompose_result.txt")
        with open(text_file_path, 'r') as file:
            txt = file.read()
        simple_strings = txt.split('\n')
        for simple_string in simple_strings:
            intermed = []
            simple_string_decomposed = simple_string.split('|')
            for i in range(len(simple_string_decomposed)):
                simple_string_decomposed[i] = simple_string_decomposed[i].strip('" .')
                intermed.append(simple_string_decomposed[i])
            simple_strings_cleaned.append(intermed)
    print(simple_strings_cleaned)

    tree_list = []
    if algebra_method == 'five':
        text_file_path = os.path.join(data_path, "flickr_500_result.txt")
        tree_list = output_tree_list(text_file_path)
    print(tree_list)
    ...
    if subset_img_id is not None:
        ...
        if algebra_method == "five":
            ...
            sub_caption_ls = []
            line_idx = 0
            tree = construct_tree_from_string('(0){Two men barbecuing on the grill with a little girl playing in background.-><1,2,> } (2){One man barbecuing on the grill [2]-><3,> } (1) { " | a little girl is playing in the background [1]" } (3) { One man is barbecuing on the grill [1] }')
    #     # Display the tree
        ...
        for selected_img_idx in range(query_count):
            print(f"We selected this img_idx: {selected_img_idx}")
            data_path = '/content/unzipped_images_fixed/flickr/'
            image_path_11 = os.path.join(data_path, "flickr30k-images/") + query_df.iloc[selected_img_idx]['filename']
            origin_file_idx = df[df['filename'] == query_df.iloc[selected_img_idx]['filename']].index[0]
            full_query = query_df.iloc[selected_img_idx]['choose']
        ...
        if algebra_method == "five":
            ...
            all_grouped_sub_q_ids_ls = [None]
        # else:
        data_path = '/content/drive/MyDrive/flickr/'
...
#data_path = /content/drive/MyDrive/flickr/
#query_path = /content/drive/MyDrive/concept_based_retrieval-main-fixed-alg-five/output/
#data_file_name = flickr_annotations_30k.csv
def load_flickr_text_datasets(data_path, query_path, img_idx_ls, cached_caption_file_name="flickr_blip_captioning.pkl", data_file_name="flickr_annotations_30k.csv"):
    cached_blip_caption_file_name = os.path.join(query_path, cached_caption_file_name)
    ...
    if os.path.exists(cached_blip_caption_file_name):
        ...
        caption_pd = pd.read_csv(os.path.join(data_path, data_file_name))
        ...
        for img_id in tqdm(img_idx_ls, desc="Loading captions"):
            img_path = '/content/unzipped_images_fixed/flickr/flickr30k-images/'
            img_file_name = os.path.join(img_path, caption_pd[caption_pd['img_id'] == img_id]["filename"].values[0])
            print(img_file_name)
            raw_image = Image.open(img_file_name).convert('RGB')
...
def get_sam_segments_for_single_image(image, sam):
    # Create a mask generator from SAM
    mask_generator = SamAutomaticMaskGenerator(sam)

    # Load the image as a numpy array
    image_numpy = np.array(image)

    # Generate masks from SAM (ensuring the image and model are on the same device)
    masks = mask_generator.generate(image_numpy)
    return masks
...
def get_sam_segments(images, sam):
    all_labels = []
    for image in tqdm(images):
        segments = get_sam_segments_for_single_image(image, sam)
        all_labels.append(segments)
    return all_labels
...
def masks_to_bboxes(masks, segment_method):
    all_bboxes = []
    ...
    for img_mask in tqdm(masks):
        ...
        elif segment_method == 'sam':
            bboxes = masks_to_bboxes_single_img_sam(img_mask)
            all_bboces.append(bboxes)
...
def masks_to_bboxes_single_img_sam(img_mask):
    bboxes = []
    for mask in img_mask:
        x, y, w, h = mask['bbox']  # SAM provides bounding boxes in [x, y, w, h]
...
def get_patches_from_bboxes(patch_emb_ls, img_per_batch_ls, masks_ls, bboxes_ls,patch_count_for_compute_ls, patch_count_ls, forward_func,model, image_file_name_ls, input_processor, image_size=(224, 224), processor=None, resize=None, device="cpu",save_mask_bbox=False, segment_method='slic',sam=None):
    # if sub_bboxes == None:
    ...
    for i, image_file_name in tqdm(enumerate(image_file_name_ls)):
        ...
        for patch_count_idx in range(len(patch_count_ls)):
            ...
            if segment_method == 'slic':
                ...
                img_mask = get_sam_segments_for_single_image(image, sam)
...
def get_patches_from_bboxes0(patch_emb_ls, img_per_batch_ls, masks_ls, bboxes_ls,patch_count_for_compute_ls, patch_count_ls, forward_func,model, image_file_name_ls, input_processor, sub_bboxes=None, image_size=(224, 224), processor=None, resize=None, device="cpu", save_mask_bbox=False, segment_method='slic',sam=None):
    # if sub_bboxes == None:
    #     sub_bboxes = [[[bbox] for bbox in img_bboxes] for img_bboxes in all_bboxes]
...
class ConceptLearner:
    ...
    def patches(self, images=None, patch_method="slic", sam=None):
        if images is None:
            images = self.samples
        ...
        if patch_method == 'slic':
            masks = get_slic_segments(images, n_segments=8 * 8)
        elif patch_method == 'sam':
            masks = get_sam_segments(images, sam)
        bboxes_for_imgs = masks_to_bboxes(masks, segment_method=patch_method)
    ...
    def get_patches(self, segmentation_method, model_name, patch_count_ls, samples_hash, img_idx_ls=None, img_file_name_ls=None, method="slic", not_normalize=False, use_mask=False, compute_img_emb=True, save_mask_bbox=False, sam=None):
        ...
        if self.img_concept:
            if True in patch_count_for_compute_ls:
                if segmentation_method == "scene_graph":
                    ...
                    if not use_mask:
                        get_patches_from_bboxes(patch_emb_ls, img_per_batch_ls, masks_ls, bboxes_ls, patch_count_for_compute_ls, patch_count_ls, self.input_to_latent, self.model, img_file_name_ls, self.input_processor, device=self.device, resize=self.image_size, save_mask_bbox=save_mask_bbox, segment_method=segmentation_method, sam=sam)
                    else:
                        get_patches_from_bboxes0(patch_emb_ls, img_per_batch_ls, masks_ls, bboxes_ls, patch_count_for_compute_ls, patch_count_ls, self.input_to_latent, self.model, img_file_name_ls, bboxes, self.input_processor, device=self.device, resize=self.image_size, save_mask_bbox=save_mask_bbox, segment_method=segmentation_method, sam=sam)
...
def convert_samples_to_concepts_img(args, samples_hash, model, img_file_name_ls, img_idx_ls, processor, device, patch_count_ls = [32], save_mask_bbox=False):
    ...
    if(args.segmentation_method == 'sam'):
        sam_model_type = "vit_l"
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        sam_model_folder = "/content/drive/MyDrive/sam_vit_l_0b3195.pth"
        sam = sam_model_registry[sam_model_type](checkpoint=sam_model_folder)
        sam = sam.to(device)
    else:
        sam = None
    ...
    res = cl.get_patches(args.segmentation_method, args.model_name, patch_count_ls, samples_hash, img_idx_ls=img_idx_ls, img_file_name_ls=img_file_name_ls, method="slic", compute_img_emb=True, save_mask_bbox=save_mask_bbox, sam=sam)
