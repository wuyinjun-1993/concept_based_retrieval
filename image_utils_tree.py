...
def load_flickr_dataset_full0(data_path, query_path, subset_img_id=None, redecompose=False, total_count = 1000, algebra_method="five"):
    ...
    if algebra_method == "five":
        ...
        '''
        '''
        (0){Two people wearing yellow jackets crosscountry skiing.-><1,> } (1){One person wearing a yellow jacket skiing. [2]-><2,> } (2) { One person wearing a yellow jacket [2]| One person is skiing. [2] }
        '''
        tree_11 = construct_tree_from_string('(0){Six males playing a game in the sand with wooden paddles.-><1,> } (1){One male playing a game in the sand with wooden paddles. [6]-><2,> } (2) { One male playing a game in the sand [6]| a game with wooden paddles [6] }')
        #tree_11 = tree_list[selected_img_idx]
    #     # Display the tree
        tree_11.display(tree_11.root)
        sub_caption_ls = [[tree_11]]
        all_grouped_sub_q_ids_ls = [None]
    elif algebra_method == "two":
        ...
        sub_caption_ls = [simple_strings_cleaned[selected_img_idx]]
        print("Here are the sub_captions:")
        print(sub_caption_ls)
        # all_grouped_sub_q_ids_ls = [[[0,1], [2,3]]]
        all_grouped_sub_q_ids_ls = [None]
    else:
        sub_caption_ls = [['One man in a gray shirt','One man in a black shirt',"an elderly woman is playing a stringed instrument"]]
