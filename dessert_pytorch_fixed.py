...
class MaxFlash:
    ...
    def get_score(self, query_hashes: torch.Tensor, num_elements: int, collision_count_to_sim: torch.Tensor) -> float:
...
class MaxFlashArray:
    ...
    def get_document_scores(self, query: torch.Tensor, documents_to_query: torch.Tensor):
        ...
        for i, flash_index in enumerate(documents_to_query):
            score = self._maxflash_array[flash_index].get_score(hashes, num_vectors_in_query, self._collision_count_to_sim)
