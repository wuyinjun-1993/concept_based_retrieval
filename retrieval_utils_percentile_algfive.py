...
def retrieve_by_embeddings(perc_methods, full_sub_queries_ls, queries, retriever, all_sub_corpus_embedding_ls, query_embeddings, qrels, query_count = 10, parallel=False, clustering_topk=500, batch_size=16,in_disk=False,doc_retrieval=None,use_clustering=False,prob_agg="prod",method="two",_nprobe_query=2, index_method="default",dataset_name="", **kwargs):
    print("results with decomposition::")
    ...
    if not use_clustering:
        ...
        if not index_method == "dessert":
            kwargs['index_method'] = index_method
            doc_retrieval._nprobe_query = _nprobe_query
            results = doc_retrieval.query_multi_queries(all_sub_corpus_embedding_ls, query_embeddings, top_k=min(clustering_topk,len(all_sub_corpus_embedding_ls)), num_to_rerank=min(clustering_topk,len(all_sub_corpus_embedding_ls)), prob_agg=prob_agg,method=method, **kwargs)
        else:
            kwargs['clustering_topk'] = clustering_topk
            results = retrieve_with_dessert(all_sub_corpus_embedding_ls, query_embeddings, doc_retrieval, prob_agg, method, **kwargs)
    ...
    if dataset_name != 'flickr':
        for perc_method in perc_methods:
            #Option 1: count of words
            if perc_method == "one":
                print("Option 1: count of words")
                query_lengths = [len(query.split()) for query in queries]
            ...
            sorted_buckets = sorted(bucket_groups.keys())
            print(f"These are the percentiles for: {perc_method}")
            for bucket in sorted_buckets:
                indices = bucket_groups[bucket]
                filtered_queries = [queries[i] for i in indices]
                filtered_qrels = {str(i + 1): qrels[str(i + 1)] for i in indices}
        ...
        for elem in full_sub_queries_ls:
            cur_tree = elem[0]
            total_nodes, count_nodes, max_tgt_count, total_tgt_count = analyze_tree(cur_tree)
            node_count_list.append(total_nodes)
            cnt_node_count_list.append(count_nodes)
            max_tgt_count_list.append(max_tgt_count)
            total_tgt_count_list.append(total_tgt_count)
        
        for perc_method in perc_methods:
            #Calculate the percentile rank for each statistic
            #Option 1: count of words
            if perc_method == "one":
                print("Option 1: count of words")
                query_lengths = [len(query.split()) for query in queries]
            ...
            if perc_method == "two":
                print("Option 2: length of sentence")
                query_lengths = [len(query) for query in queries]
            elif perc_method == 'four':
                print("Option 4: total number of nodes")
                query_lengths = node_count_list
            elif perc_method == 'five':
                print("Option 5: total number of count nodes")
                query_lengths = cnt_node_count_list
            elif perc_method == 'six':
                print("Option 6: max tgt count")
                query_lengths = max_tgt_count_list
            elif perc_method == 'seven':
                print("Option 7: total tgt count")
                query_lengths = total_tgt_count_list
