...
def load_flickr_dataset_full(data_path, query_path, subset_img_id=None, redecompose=False, total_count = 1000, algebra_method="five"):
    ...
    print(f"The selected img id is: {subset_img_id}")
    
    # full_query = "Five ballet dancers caught mid jump in a dancing studio with sunlight coming through a window"
    #full_query = "Several women are gather around a table in a corner surrounded by bookshelves." #change
    full_query = df.iloc[selected_img_idx]['raw'].split('", "')[0][2:]
    print(f"This is the full query: {full_query}")
    # selected_img_idx = 11
    
    img_folder_path = '/content/unzipped_images_fixed/flickr/'
    image_path_11 = os.path.join(img_folder_path, "flickr30k-images/") + df.iloc[selected_img_idx]['filename']
    #image_path_11 = os.path.join(img_folder_path, "flickr30k-images/") + df[df['img_id']==selected_img_idx]['filename']
    print(f"With image path {image_path_11}")
