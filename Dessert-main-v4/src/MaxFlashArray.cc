...
std::vector<float> MaxFlashArray<LABEL_T>::getDocumentScores(
    ...
    const std::vector<uint32_t>& documents_to_query,
...
#pragma omp for
    for (uint64_t i = 0; i < result.size(); i++) {
      ...
      result[i] = _maxflash_array.at(flash_index)
                      ->getScore(hashes, num_vectors_in_query, buffer, _collision_count_to_sim, prob_agg, is_img_retrieval);
...
template <typename LABEL_T>
std::vector<float> MaxFlashArray<LABEL_T>::getDocumentScoresDependencies(
      const EmbeddingArray& query,
      const std::vector<uint32_t>& documents_to_query,
      const std::string& prob_agg,
      uint32_t query_idx,
      ...
      uint32_t dependency_topk) const
  {
    //std::cout << "Inside getDocumentScoresDependencies" << std::endl;
    const std::vector<uint32_t> hashes = hash(query);
    uint32_t num_vectors_in_query = query.request().shape[0];

    std::vector<float> result(documents_to_query.size());
    std::vector<std::vector<std::vector<uint32_t>>> partial_grouped_sub_q_ids_ls = grouped_sub_q_ids_ls[query_idx];
    
    std::vector<std::vector<std::vector<uint32_t>>> bboxes_overlap_ls_subset;
    for (uint64_t i = 0; i < result.size(); i++) {
        uint64_t flash_index = documents_to_query.at(i);
        bboxes_overlap_ls_subset.push_back(bboxes_overlap_ls[flash_index]);
    }
    //std::cout << "Ready for OMP loop" << std::endl;
#pragma omp parallel default(none) \
    shared(result, documents_to_query, hashes, query, num_vectors_in_query, prob_agg, query_sub_idx, dependency_topk, is_img_retrieval, partial_grouped_sub_q_ids_ls, bboxes_overlap_ls_subset)
    {
      std::vector<uint32_t> buffer(_max_allowable_doc_size);
...
#pragma omp for
      for (uint64_t i = 0; i < result.size(); i++)
      {
        ...
        {
          result[i] = _maxflash_array.at(flash_index)->getScoreDependency(hashes, num_vectors_in_query, buffer, _collision_count_to_sim, query_sub_idx, partial_grouped_sub_q_ids_ls, bboxes_overlap_ls_subset[i], dependency_topk, prob_agg, is_img_retrieval);
        }
        result[i] /= num_vectors_in_query;
      }
    }
