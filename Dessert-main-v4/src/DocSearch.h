...
class DocSearch {
 public:
  //good
  DocSearch(uint32_t hashes_per_table, uint32_t num_tables, uint32_t dense_dim, uint32_t nprobe_query,
            const Centroids& centroids_input)
      : _dense_dim(dense_dim),
        _nprobe_query(nprobe_query),   
  ...
  std::pair<std::vector<float>, std::vector<uint32_t>> query(
      const EmbeddingArray& embeddings, uint32_t top_k,
      uint32_t num_to_rerank, const std::string& prob_agg,
      bool use_frequency, bool is_img_retrieval) {
    std::vector<uint32_t> centroid_ids =
        getNearestCentroids(embeddings, _nprobe_query);
    return queryWithCentroids(embeddings, centroid_ids, top_k, num_to_rerank, prob_agg, use_frequency, is_img_retrieval);
  }

  std::pair<std::vector<float>, std::vector<uint32_t>> querywithdependency(
      const EmbeddingArray& embeddings, uint32_t top_k,
      uint32_t num_to_rerank, const std::string& prob_agg,
      bool use_frequency,
      bool is_img_retrieval,
      uint32_t dependency_topk,
      uint32_t query_idx,
      uint32_t query_sub_idx,
      const std::vector<std::vector<std::vector<std::vector<uint32_t>>>>& grouped_sub_q_ids_ls,
      const std::vector<std::vector<std::vector<uint32_t>>>& bboxes_overlap_ls) {
    std::vector<uint32_t> centroid_ids =
        getNearestCentroids(embeddings, _nprobe_query);
    return queryWithCentroidsDependency(embeddings, centroid_ids, top_k, num_to_rerank, prob_agg, use_frequency,
    is_img_retrieval, dependency_topk, query_idx, query_sub_idx, grouped_sub_q_ids_ls, bboxes_overlap_ls);
  }

  std::pair<std::vector<float>, std::vector<uint32_t>> queryWithCentroids(
      const EmbeddingArray& embeddings, const std::vector<uint32_t>& centroid_ids,
      uint32_t top_k, uint32_t num_to_rerank, const std::string& prob_agg, bool use_frequency,
      bool is_img_retrieval) {
    ...
    if (use_frequency == true) {
        ...
        top_k_internal_ids =
        nofrequencyCountCentroidBuckets(centroid_ids, num_to_rerank);
    }
    
    std::vector<float> reranked =
        rankDocuments(embeddings, top_k_internal_ids, prob_agg, is_img_retrieval);

    return {reranked, top_k_internal_ids};
  }
  
  std::pair<std::vector<float>, std::vector<uint32_t>> queryWithCentroidsDependency(
      const EmbeddingArray& embeddings, const std::vector<uint32_t>& centroid_ids,
      uint32_t top_k, uint32_t num_to_rerank, const std::string& prob_agg, bool use_frequency,
      bool is_img_retrieval,
      uint32_t dependency_topk,
    ...
    std::vector<float> reranked =
        rankDocumentsDependency(embeddings, top_k_internal_ids, prob_agg, is_img_retrieval, dependency_topk,
        query_idx, query_sub_idx, grouped_sub_q_ids_ls, bboxes_overlap_ls);
  ...
  //good
  static std::unique_ptr<DocSearch> deserialize(
      const std::string& path) {
    std::ifstream filestream(path, std::ios::binary);
    cereal::BinaryInputArchive iarchive(filestream);
    std::unique_ptr<DocSearch> serialize_into(new DocSearch());
  ...
  std::vector<float> rankDocuments(
      const EmbeddingArray& query_embeddings,
      const std::vector<uint32_t>& internal_ids_to_rerank, const std::string& prob_agg,
    ...
    std::vector<float> document_scores = _document_array->getDocumentScores(
        query_embeddings, internal_ids_to_rerank, prob_agg, is_img_retrieval);

    return document_scores;
  }
  
  std::vector<float> rankDocumentsDependency(
      const EmbeddingArray& query_embeddings,
      const std::vector<uint32_t>& internal_ids_to_rerank, const std::string& prob_agg,
      bool is_img_retrieval,
      uint32_t dependency_topk,
      ...
      uint32_t query_sub_idx,
      const std::vector<std::vector<std::vector<std::vector<uint32_t>>>>& grouped_sub_q_ids_ls,
      const std::vector<std::vector<std::vector<uint32_t>>>& bboxes_overlap_ls) const {
    // This returns a vector of scores, where the ith score is the score of
    // the document with the internal_id at internal_ids_to_rerank[i]
    std::vector<float> document_scores = _document_array->getDocumentScoresDependencies(
        query_embeddings, internal_ids_to_rerank, prob_agg,
        query_idx, query_sub_idx, is_img_retrieval, grouped_sub_q_ids_ls, bboxes_overlap_ls, dependency_topk);
...
#pragma omp parallel for default(none) \
  ...
  std::vector<uint32_t> nofrequencyCountCentroidBuckets(
      const std::vector<uint32_t>& centroid_ids, uint32_t top_k) {
    
    // Create an empty set to hold unique internal_ids
    std::set<uint32_t> unique_internal_ids;

    // Iterate over the centroid_ids and their corresponding internal_ids
    for (uint32_t centroid_id : centroid_ids) {
        for (uint32_t internal_id : _centroid_id_to_internal_id.at(centroid_id)) {
            unique_internal_ids.insert(internal_id);
        }
    }
    
    // Convert the set to an equivalent vector
    std::vector<uint32_t> result(unique_internal_ids.begin(), unique_internal_ids.end());
  ...
  void serialize(Archive& archive) {
    archive(_dense_dim, _nprobe_query, _largest_internal_id, _num_centroids,
            _document_array, _centroids, _centroid_id_to_internal_id, _internal_id_to_doc_id);
  }
};
