    ...
    def processing_one_count_node(self, corpus_idx, score_function, head_count_node, image_patch_embeddings, image_containment_list, bboxes_overlap_ls, sub_q_ls_idx, agg_method, query_itr, device, valid_idx=None):
        ...
        for idx in range(0, CNT):
            ...
            while all_count_nodes:
                ...
                sim_2, matched_segs = self.processing_one_count_node(corpus_idx, score_function, count_node, image_patch_embeddings, image_containment_list, bboxes_overlap_ls, sub_q_ls_idx,agg_method, query_itr, device, valid_idx=valid_idx)
                Match_Seg_ls.extend(matched_segs)
                if agg_method == 'sum':
                    sim_1 = sim_1 + sim_2
                elif agg_method == 'prod':
