...
do
	command="python main.py --dataset_name ${dataset_name} --data_path ${data_path} --query_count -1 --total_count -1 --query_concept --img_concept --algebra_method ${method}  --search_by_cluster --clustering_topk 1000  --clustering_number 0.001  --clustering_doc_count_factor 200 --nprobe_query ${nprobe_query} --dependency_topk 20 --subset_patch_count ${val}"
        echo "$command"
	$command > ${data_path}/${dataset_name}/output_full_query_full_data_clustering_method_${method}_patch_count_${val}_dependency_topk_20.txt 2>&1 
